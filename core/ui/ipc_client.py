...
class IPCClientUI(UIBase):
    ...
    async def send_message(self, message: str, source: str = None):
        await self._send({  # Change this line
            "type": "message",
            "data": {
                "message": message,
                "source": source
    ...
    async def send_diff(self, file_path: str, old_content: str, new_content: str):
    ...
    async def send_stream(self, content: str):
        await self._send_json({"type": "stream", "content": content})

    async def send_testing_instructions(self, instructions: str):
        await self._send_json({"type": "testing_instructions", "content": instructions})

    async def send_server_logs(self, logs: str):
        await self._send_json({"type": "server_logs", "content": logs})

    async def send_app_progress(self, progress: str):
        await self._send_json({"type": "app_progress", "content": progress})

    async def send_deployment_info(self, info: str):
        await self._send_json({"type": "deployment_info", "content": info})
