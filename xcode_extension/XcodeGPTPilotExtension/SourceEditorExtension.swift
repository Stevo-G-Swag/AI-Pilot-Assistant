...
class SourceEditorExtension: NSObject, XCSourceEditorExtension {
    ...
    func setupSocket() {
        manager = SocketManager(
            socketURL: URL(string: "http://localhost:5000")!, config: [.log(true), .compress])
        ...
        socket?.on("code_update") { data, ack in
                ...
                let filePath = codeData["file_path"] as? String
            {
                NotificationCenter.default.post(
                    name: .codeUpdated, object: nil,
                    userInfo: ["code": code, "file_path": filePath])
            }
        }
    ...
    func perform(
        ...
        guard let buffer = invocation.buffer else {
            ...
            completionHandler(
                NSError(
                    domain: "XcodeGPTPilot", code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "Unable to access buffer"]))
            return
        }

        let selectedText =
            buffer.selections.compactMap { $0 as? XCSourceTextRange }.first.flatMap { range in
                buffer.lines.compactMap { $0 as? String }[range.start.line...range.end.line].joined(
                    separator: "\n")
            } ?? ""
    ...
    private func insertGeneratedCode(_ code: String, into buffer: XCSourceTextBuffer) {
        ...
        let newRange = XCSourceTextRange(
            start: XCSourceTextPosition(line: insertionIndex, column: 0),
            end: XCSourceTextPosition(
                line: insertionIndex + code.components(separatedby: "\n").count + 2, column: 0))
    ...
    func generateCode(context: String, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        let body: [String: Any] = [
            "context": context, "prompt": "Generate code based on this context",
            "model": selectedModel,
        ]
            ...
            do {
                    ...
                    let generatedCode = json["generated_code"] as? String
                {
                    ...
                    completion(
                        .failure(
                            NSError(
                                domain: "XcodeGPTPilot", code: 7,
                                userInfo: [
                                    NSLocalizedDescriptionKey: "Invalid response format from server"
                                ])))
                }
            } catch {
                completion(
...
class RefactorCodeCommand: NSObject, XCSourceEditorCommand {
    ...
    private func insertRefactoringSuggestions(
        _ suggestions: String, into buffer: XCSourceTextBuffer
    ) {
        let insertionIndex = buffer.selections.last!.end.line + 1
        ...
        let newRange = XCSourceTextRange(
            start: XCSourceTextPosition(line: insertionIndex, column: 0),
                ...
                line: insertionIndex + suggestions.components(separatedby: "\n").count + 2,
                column: 0))
        buffer.selections.removeAllObjects()
        buffer.selections.add(newRange)
    }
    ...
    func suggestRefactoring(
        codeSnippet: String, completion: @escaping (Result<String, Error>) -> Void
    ) {
...
struct SettingsView: View {
    @State private var selectedModel =
        UserDefaults.standard.string(forKey: "SelectedModel") ?? "openai/gpt-4o"
...
struct CollaborationView: View {
        ...
        VStack {
            ...
            TextEditor(text: $currentCode)
                ...
                .onChange(of: currentCode) { newValue in
                    SourceEditorExtension.shared.socket?.emit(
                        "code_update",
                        ["code": newValue, "file_path": invocation.buffer.contentUTI])
                }
    ...
    func setupNotifications() {
        NotificationCenter.default.addObserver(
            forName: .connectedUsersUpdated, object: nil, queue: .main
        ) { _ in
            self.connectedUsers = SourceEditorExtension.shared.connectedUsers
        }

        NotificationCenter.default.addObserver(forName: .codeUpdated, object: nil, queue: .main) {
            notification in
        ...
        NotificationCenter.default.addObserver(
            forName: .chatMessageReceived, object: nil, queue: .main
        ) { notification in
            if let userInfo = notification.userInfo,
    ...
    func sendChatMessage() {
        ...
        SourceEditorExtension.shared.socket?.emit(
            "chat_message", ["user": "Xcode User", "message": currentMessage])
        currentMessage = ""
    }
}
...
extension Notification.Name {
    ...
    /// Notification indicating that the code has been updated.
    static let codeUpdated = Notification.Name("XcodeGPTPilotExtension.codeUpdated")
    
    /// Notification indicating that a new chat message has been received.
    static let chatMessageReceived = Notification.Name("XcodeGPTPilotExtension.chat_message")
    }
    #endif
